...
class PedestrianDatasetCamDropout(VisionDataset):
    def __init__(
        ...
        self.base = base
        self.root, self.num_cam, self.num_frame = base.root, num_cameras, base.num_frame  # base.num_cam, base.num_frame
    ...
    def prepare_gt(self):
        ...
        for fname in sorted(os.listdir(os.path.join(self.root, 'annotations_positions'))):
            ...
            for single_pedestrian in all_pedestrians:
                ...
                in_cam_range = sum(is_in_cam(cam) for cam in self.base.test_cameras)  # range(self.num_cam))
                if not in_cam_range:
                    continue
    ...
    def get_bev_gt(self, mem_pts, pids, img_ids):
        ...
        for pts, pid in zip(mem_pts, pids):
            if pid in img_ids:  # enshour that the used cameras can see the pedestrian
                ct = pts[:2]
                ct_int = ct.int()
    ...
    def __getitem__(self, index):
        ...
        n = self.num_cam
        cameras = random.sample(cameras, n)
        if not self.mix_views:
            cameras.sort()  # I do not know if it will work if the camera index's are not sorted
        # images
        ...
        # get all image ids, those ids will be used to creat bev gt
        img_ids = np.unique(pids_img)
        img_ids = img_ids[img_ids != 0]
        img_ids.sort()

        mem_pts = self.vox_util.Ref2Mem(worldgrid_pts, self.Y, self.Z, self.X)
        center_bev, valid_bev, pid_bev, offset_bev = self.get_bev_gt(mem_pts[0], world_pids,img_ids)

        # # print(cameras)
        # # print(center_bev.shape)
        # img_ids = np.unique(pids_img)
        # img_ids = img_ids[img_ids != 0]
        # img_ids.sort()
        # world_pids.sort()
        # print(cameras, self.frame_path)
        # print('image ids', img_ids)
        # print('world ids', world_pids)
        # plt.imshow(center_bev.permute(1, 2, 0).cpu())
        # plt.title(f'{cameras}, {index}, {self.frame_path}')
        # plt.show()
        # exit()
        ...
        item = {
            ...
            'extras': {'views': cameras, 'frame_path': self.frame_path}
        }
