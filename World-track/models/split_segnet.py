...
class SplitSegnet(nn.Module):
    def __init__(self, Y, Z, X,
        ...
        super(SplitSegnet, self).__init__()
        ...
        if self.num_cameras is not None:
            if not self.use_avgpool:
                self.cam_compressor = nn.Sequential(
                    nn.Conv3d((feat2d_dim + add_dimension) * self.num_cameras, feat2d_dim,
        ...
        if self.output_BEV:
            self.bev_compressor = nn.Sequential(
                nn.Conv2d((feat2d_dim + add_dimension) * self.Z, latent_dim, kernel_size=3, padding=1),
    ...
    def feature_weighting(self, out_image_dict, feat_cams_, waighting_mode='multiply'):
        ...
        assert waighting_mode in ['multiply', 'concat', 'no']
    ...
    def forward(self, rgb_cams, pix_T_cams, cams_T_global, vox_util, ref_T_global, prev_bev=None):
        ...
        show_images = False
        if show_images:
            intermidiat_showing(out_image_dict, rgb_cams_)

        # weight image featurs with the output of the detection head
        ...
        #
        # print('\n')
        # print(feat_mems_.shape)
        # print(feat_direction_.shape)
        #
        # exit()
        ...
        if self.output_BEV:
            ...
            feat_bev_ = feat_mem.permute(0, 1, 3, 2, 4).reshape(B, feat2D_dim * Z, Y, X)
...
def intermidiat_showing(out_image_dict, rgb_cams_):
    ...
    imaged = rgb_cams_[image_view].permute(1, 2, 0).cpu()  # torch.mean(rgb_cams_, dim=1)  # .permute(1, 2, 0).cpu()
    ...
    body_and_head = utils.basic._sigmoid(imaged_center_0) + utils.basic._sigmoid(imaged_center_1)
    ...
    # Sum the results
    body_and_head_new = sigmoid_center_0 + sigmoid_center_1
    body_and_head_new = body_and_head_new[image_view]
    import os
    plt.subplot(221)
    ...
    plt.subplot(223)
    plt.imshow(body_and_head_new.cpu().numpy())
    plt.subplot(224)
